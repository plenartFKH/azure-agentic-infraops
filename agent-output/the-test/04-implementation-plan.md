...
- `allTags = union(requiredRgTags, { ManagedBy: 'Bicep', Project: 'the-test' })`
...
- Spoke VNet (`vnet-spoke-the-test-{env}`, `10.1.0.0/16`)
...
- Log Analytics Workspace (`log-the-test-{env}`)
  - Retention: 365 days (PCI-DSS § 10.7)
  - SKU: PerGB2018
- Application Insights (`appi-the-test-{env}`)
...
- User Assigned Managed Identity (`id-the-test-{env}`)
...
- Public IP (`pip-fw-the-test-{env}`, Standard, Static, Zone 1,2,3)
- Azure Firewall Premium (`fw-the-test-{env}`)
...
- Public IP (`pip-bas-the-test-{env}`, Standard, Static)
- Bastion Host Standard (`bas-the-test-{env}`)
...
- PostgreSQL Flexible Server (`psql-the-test-{env}`)
...
- Cosmos DB Account (`cosmos-the-test-{env}`)
  - API: NoSQL
  - Consistency: Session
...
- Service Bus Namespace Premium (`sb-the-test-{env}`)
  - 1 Messaging Unit
  - Zone-redundant
  - Private endpoint in `snet-pe`
...
- AKS Managed Cluster Standard (`aks-the-test-{env}`)
...
- Front Door Premium (`fd-the-test-{env}`)
...
- APIM Premium (`apim-the-test-{env}`)
  - 1 unit
...
```bash
az network vnet peering list -g rg-the-test-{env} --vnet-name vnet-hub-the-test-{env}
az monitor log-analytics workspace show -g rg-the-test-{env} -n log-the-test-{env}
```
...
az network firewall show -g rg-the-test-{env} -n fw-the-test-{env} --query "sku.tier"
```
...
```bash
az postgres flexible-server show -g rg-the-test-{env} -n psql-the-test-{env} \
  --query "highAvailability"
az cosmosdb show -g rg-the-test-{env} -n cosmos-the-test-{env} \
...
```bash
az aks show -g rg-the-test-{env} -n aks-the-test-{env} \
  --query "{pools:agentPoolProfiles[].{name:name,count:count,vmSize:vmSize}}"
az aks addon show -g rg-the-test-{env} -n aks-the-test-{env} -a keda
```
...
```bash
az afd profile show -g rg-the-test-{env} --profile-name fd-the-test-{env}
az apim show -g rg-the-test-{env} -n apim-the-test-{env} \
...
|----------|---------|---------|
| Resource Group | `rg-{project}-{env}` | `rg-the-test-prod` |
| Hub VNet | `vnet-hub-{project}-{env}` | `vnet-hub-the-test-prod` |
| Spoke VNet | `vnet-spoke-{project}-{env}` | `vnet-spoke-the-test-prod` |
| Subnet | `snet-{purpose}-{env}` | `snet-aks-cde-prod` |
| NSG | `nsg-{purpose}-{env}` | `nsg-aks-cde-prod` |
| DDoS Plan | `ddos-{project}-{env}` | `ddos-the-test-prod` |
| Log Analytics | `log-{project}-{env}` | `log-the-test-prod` |
| App Insights | `appi-{project}-{env}` | `appi-the-test-prod` |
| Managed Identity | `id-{project}-{env}` | `id-the-test-prod` |
| Key Vault | `kv-{short}-{env}-{suffix}` | `kv-thetest-prod-a1b2` |
| Azure Firewall | `fw-{project}-{env}` | `fw-the-test-prod` |
| Bastion | `bas-{project}-{env}` | `bas-the-test-prod` |
| Public IP | `pip-{purpose}-{project}-{env}` | `pip-fw-the-test-prod` |
| PostgreSQL | `psql-{project}-{env}` | `psql-the-test-prod` |
| Cosmos DB | `cosmos-{project}-{env}` | `cosmos-the-test-prod` |
| Service Bus | `sb-{project}-{env}` | `sb-the-test-prod` |
| Container Registry | `cr{short}{env}{suffix}` | `crthetestproda1b2` |
| AKS | `aks-{project}-{env}` | `aks-the-test-prod` |
| Front Door | `fd-{project}-{env}` | `fd-the-test-prod` |
| APIM | `apim-{project}-{env}` | `apim-the-test-prod` |
